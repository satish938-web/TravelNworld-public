...
const ItineraryCard = ({ itinerary, destinationId, index }) => {
  ...
  return (
    <motion.div
      ...
      onClick={() => navigate(`/international-itinerary-detail/${destinationId}/${itinerary.slug || itinerary._id}`)}
    ...
    >
      ...
      <div className="flex-1 p-8 flex flex-col">
        ...
        <p className="text-gray-400 text-xs font-medium leading-relaxed mb-6 line-clamp-2">
          {itinerary.shortDescription || "Experience the ultimate international getaway with our expertly curated travel plan."}
        </p>
    ...
    const fetchAll = async () => {
      try {
        const [res1, res2] = await Promise.all([
          getJson(`/api/itineraries?type=international&destination=${encodeURIComponent(destinationId)}`),
          getJson(`/api/agent-itineraries?type=international&destination=${encodeURIComponent(destinationId)}`)
        ]);
      ...
      <div className="relative pt-32 pb-24 px-6 overflow-hidden bg-black">
        ...
        <div className="relative z-10 max-w-7xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-red-600 font-black uppercase tracking-[0.6em] text-[10px]">Worldwide Luxury</span>
            <span className="w-12 h-[2px] bg-red-600 rounded-full" />
          </motion.div>
          ...
          >
            Curated selection of premium international tour packages for {destinationName}. 
            Explore unique experiences from the world's top travel specialists.
          </motion.p>
        </div>
      </div>
        ...
        {!loading && !error && itineraries.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-400 max-w-sm mb-10 font-medium">
              We haven't listed any international packages for {destinationName} yet. 
